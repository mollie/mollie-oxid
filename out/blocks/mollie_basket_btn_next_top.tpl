...
[{if $oViewConf->mollieShowApplePayButtonOnBasket() || $oViewConf->mollieShowPayPalExpressButtonOnBasket()}]
    ...
    [{if $oViewConf->mollieShowPayPalExpressButtonOnBasket()}]
        [{oxid_include_dynamic file="molliepaypalexpress.tpl" type="mollie" position="BasketTop" payment_price=$oViewConf->mollieGetApplePayBasketSum() delivery_costs=$oxcmp_basket->getDeliveryCosts() shipping_id=$oxcmp_basket->getShippingId()}]
    [{/if}]
[{/if}]
