 ...
 */
class EachPromise implements \_PhpScoper5c46f59d284a1\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \_PhpScoper5c46f59d284a1\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \_PhpScoper5c46f59d284a1\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
